...
The bridge (`server.js`, ~240 lines) implements that contract on the IS side and speaks proper OIDC — `private_key_jwt` + PKCE + JWS UserInfo verification — on the eSignet side.
...
| `server.js`, `genKeys.js`, `package.json` | **The only hand-written code.** Node.js ESM, `express` + `jose`. |
| `esignet/` | Vendored upstream clone of `mosip/esignet` at tag `v1.8.0`, created here by `setup`. **Read-only.** |
| `wso2is-7.3.0/` | Unpacked WSO2 IS distribution, created here by `setup`. Config in `repository/conf/deployment.toml`. |
| `../setup-without-bridge/` | The same demo built without this bridge, as a custom federated authenticator JAR. |
| `../CLAUDE.md` | Guidance for Claude Code sessions in this repo. |

> 🚫 **Never edit `esignet/`.** It is upstream source, present only as evidence — the runbook's Appendix A maps each technical claim to a specific file in it (`TokenRequest.java`, `UserInfoResponseHelper.java`, `application-*.properties`).
...
WSO2 IS config lives in `wso2is-7.3.0/repository/conf/deployment.toml`. The session idle timeout defaults to **15 minutes**, short enough to expire between rehearsal and the real demo:
